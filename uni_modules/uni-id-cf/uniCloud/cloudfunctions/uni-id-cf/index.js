	...
	switch (action) { //根据action的值执行对应的操作
		...
		case 'login':
			...
			if (!needCaptcha || passed) {
				...
				if (res.userInfo && res.userInfo.avatar_file && res.userInfo.avatar_file.url) {
					res.userInfo.avatar_file.tempUrl = await uniCloud.getTempFileURL({
						fileList: [res.userInfo.avatar_file.url]
					}).then(res => {
